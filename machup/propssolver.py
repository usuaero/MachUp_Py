...
class PropsSolver:
    ...
    def __init__(self, plane):
        ...
        for prop in props:
            self._prop_models[prop.name] = PropModel(prop,plane.get_cg_location(),plane.get_units())
            self._total_nodes+= prop._nodes


    def solve(self,aero_state = None,prop_state = None,control_points = None):
        ...
        control_points : array
            Array containing the points in 3D space at which an induced
            velocity is to be calculated. Most typically, this array contains
            the control points used in the lifting line analysis.
        ...
        velocity_array : array
            If control_points array is given, returns an array containing
            containing the density and induced velocity at each point in 
            the order: [rho, Vx, Vy, Vz]. If no control_points array is given, 
            returns None.
        ...
        if control_points is not None:
            velocity_array = self._initialize_velocity_field(aero_state,control_points)
            for prop in self._prop_models:
                pm = self._prop_models[prop]
                pm._find_points_in_stream(control_points)
                velocities = pm._calc_velocities(control_points)
    ...
    def _calc_forces(self,aero_state = None,prop_state = None):
        ...
        self._results['FS'] = side
        self._results['FX'] = self._F[0]
        self._results['FY'] = self._F[1]
        self._results['FZ'] = self._F[2]
        self._results['MX'] = self._M[0]
        self._results['MY'] = self._M[1]
        self._results['MZ'] = self._M[2]
    ...
    def _initialize_velocity_field(self,aero_state,control_points):
        if aero_state:
            a = m.radians(aero_state.get("alpha",0.))
            b = m.radians(aero_state.get("beta",0.))
            ca = m.cos(a)
        ...
        velocity_array = np.empty([control_points.shape[0],4])
    ...
    def _distributions(self):
        ...
        for prop in self._prop_models:
            ...
            self._data['pitch'][cur_slice] = pm.get_aero_pitch_angle()
            self._data['advance_angle'][cur_slice] = pm.get_advance_angle()
            self._data['induced_angle'][cur_slice] = pm.get_induced_angle()
            self._data['alpha'][cur_slice] = pm.get_alpha()
